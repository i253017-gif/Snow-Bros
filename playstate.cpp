...
void playState::handleInput(inputManager& input)
{
	...
	{											//!wasSpacePressed makes sure last press wasnt a space
		...
		snowball[SBnum] = Snowball(startX, startY, player.getFacingDirection(), sb_speed, sb_range);
	...
	//////////===========AADDED BY ISHMALLLLLLLLLLLLL=============/////

	
		// powr up spawn when enemy dies
	
	std::string types[] = { "speed", "snowball", "distance", "balloon" };
	for (int e = 0; e < Enum; e++)
	{
		if (enemies[e]->get_hp() <= 0 && PUnum < 50)
		{
...
type = "speed";
				...
				type = "balloon";
			// its js so that power up will spawn at the position of enemy death
			sf::FloatRect enemybound = enemies[e]->getBounds();
			powerups[PUnum++] = new PowerUp_funcs(type, enemybound.left, enemybound.top);
		}
	}
	///////////=======ADDED BY ISHMAL================/////////
	// 
	...
	{
		// creating hitboxbox for power ups 
		sf::FloatRect PUbounds(powerups[i]->get_pos_x(),
			powerups[i]->get_pos_y(), 30, 30);

		if (player.getBounds().intersects(PUbounds))
		{
			// apply power up effect to player
			powerups[i]->apply(player);
			// add 100 points when power up is collected
			score_mgr.update_score(100);
			// remove collected power up from array
			delete powerups[i];
			powerups[i] = powerups[--PUnum];
		}
	}
}
...
void playState::update()
{
	...
	//////=======ADDED BY ISHMALLLLLLLLLL=========//////
	
	// UPDATE POWER-UP TIMERS - remove when expired
	
	for (int i = 0; i < PUnum; i++)
	{
		powerups[i]->update(0.016f);
		// removes power up if timer is up and also expires the effect on player
		if (powerups[i]->get_timer() == 0)
		{	powerups[i]->expire(player);
			delete powerups[i];
			powerups[i] = powerups[--PUnum];
			i--;  // it sadjust index afte power up is removed so we dont skip the next one
		}
	}
}
